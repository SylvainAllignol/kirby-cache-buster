...
```bash
composer require allignol/cache-buster
```
...
```php
// Cache buster (auto-detects extension)
<?= cacheBust('assets/css/style.css') ?>
// Output: assets/css/style.a1b2c3d4.css

<?= cacheBust('assets/js/script.js') ?>
// Output: assets/js/script.e5f6g7h8.js

// Works with absolute URLs too (e.g., from asset()->url())
<?= cacheBust(asset('images/logo.svg')->url()) ?>
// Output: https://yoursite.com/assets/images/logo.a1b2c3d4.svg

// Get only the hash
<?= cacheBustHash('assets/css/style.css') ?>
// Output: a1b2c3d4
```

### Field Methods

Use cache buster on field values when you store file paths in custom fields.
...
```php
// Custom stylesheet
<?php if ($url = $page->customStylesheet()->cacheBust()): ?>
    <link href="<?= $url ?>" rel="stylesheet">
<?php endif ?>

// PDF download link
<?php if ($url = $page->brochurePdf()->cacheBust()): ?>
...
For advanced usage, you can use the `CacheBuster` class directly:
...
// Get busted URL
$url = $cacheBuster->bust('assets/css/style.css');

// Check if active
if ($cacheBuster->isActive()) {
    // ...
}

// Get current method
$method = $cacheBuster->method();

// Get all options
$options = $cacheBuster->options();
```
